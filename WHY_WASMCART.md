...
- **True sandbox** - WASM provides real security guarantees: no filesystem access, no syscalls, and no cart-initiated network beyond manifest-declared domains (a host may hand a cart peer connections it chose itself). Run untrusted carts safely.
...
- **Port existing games easily** - the SDL2 backend approach lets existing SDL2/C games run with minimal new code. Dozens of open-source games and engines already run as carts, including DOOM (doomgeneric), Neverball, OpenArena, Extreme Tux Racer, Flare, and full Godot 4 engine games.
...
- **Opt-in complexity** - a minimal cart is ~40 lines of C. Need mouse or touch? Set `WC_FLAG_POINTER` in the cart's own info struct. Need to dial a server? Set `WC_FLAG_NET_PEER` and allowlist the domain in the manifest — one `wc_peer_*` family covers WebSocket, data channels, and whatever transport a host brings. The ABI grows with the game's needs, not upfront.
